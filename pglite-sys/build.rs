...
fn main() {
    // link to pre-built postgres
    let backend_lib_path = make_backend_lib();
    println!("cargo:rustc-link-search={}", backend_lib_path.to_str().unwrap());
    println!("cargo:rustc-link-lib=pglite_backend");

    let common_lib_path = postgres_build_dir().join("src/common");
    println!("cargo:rustc-link-search={}", common_lib_path.to_str().unwrap());
    println!("cargo:rustc-link-lib=pgcommon_srv");

    let common_lib_path = postgres_build_dir().join("src/port");
    println!("cargo:rustc-link-search={}", common_lib_path.to_str().unwrap());
...
fn gen_bindings() -> PathBuf {
    ...
    bindgen::Builder::default()
        .header("bindings.h")
        .clang_arg(format!("-I{}/src/include/", postgres_source_dir().to_str().unwrap()))
        .clang_arg(format!("-I{}/src/include/", postgres_build_dir().to_str().unwrap()))
...
fn make_backend_lib() -> PathBuf {
    let lib_out_path = out_dir().join("libpglite_backend.a");

    let mut cmd = std::process::Command::new("x86_64-linux-musl-ar");

    let cmd = cmd
        .arg("crs")
        .arg(lib_out_path.as_os_str());

    let cmd = postgres_backend_objs()
        .iter()
        .fold(cmd, |cmd, obj_path| cmd.arg(obj_path.as_os_str()));
...
fn postgres_build_dir() -> PathBuf {
    source_root().join("postgres-build")
}
...
fn postgres_backend_objs() -> Vec<PathBuf> {
    let backend_dir = postgres_build_dir().join("src/backend");

    POSTGRES_BACKEND_OBJS.iter()
        .map(|path| backend_dir.join(path))
        .collect()
}

static POSTGRES_BACKEND_OBJS: &[&str] = &[
    "access/brin/brin.o",
